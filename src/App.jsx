...
// Equilibria for an autonomous ODE y' = f(y) live where f vanishes.
// We sample at several t-values rather than just t=0 — if the user has forced
// autonomous mode on an equation that actually depends on t (e.g. sin(t)·cos(y)
// at t=0 is identically zero in y), evaluating only at t=0 produces phantom
// equilibria everywhere. Requiring f(t,y*) ≈ 0 at all sample t-values rejects
// those phantoms.
function findEquilibria(code, yMin, yMax, tSamples = [-1.7, -0.3, 0.5, 1.1, 2.3]) {
  const N = 800
  // For each t sample, find candidate roots in y, then keep only the y values
  // that show up (within tolerance) for ALL t samples.
  const tol = (yMax - yMin) * 1e-3

  const findRootsAtT = (t) => {
    const samples = new Float64Array(N + 1)
    for (let i = 0; i <= N; i++) {
      const y = yMin + ((yMax - yMin) * i) / N
      samples[i] = evalF(code, t, y)
    }
    // Reject if the entire row is ~zero — that signals f is not really a function
    // of y at this t (constant in y), not that every y is an equilibrium.
    let maxAbs = 0
    for (let i = 0; i <= N; i++) if (isFinite(samples[i])) maxAbs = Math.max(maxAbs, Math.abs(samples[i]))
    if (maxAbs < 1e-12) return null  // signal "useless sample, skip"
  ...
  // Gather root sets for each t sample; skip any that returned null (degenerate).
  const rootSets = []
  for (const t of tSamples) {
    const rs = findRootsAtT(t)
    if (rs !== null) rootSets.push(rs)
  }
  if (rootSets.length === 0) return []

  // Intersection: keep only y values that appear (within tol) in ALL sets.
  const seed = rootSets[0]
  const accepted = seed.filter(y => rootSets.every(set => set.some(r => Math.abs(r - y) < tol)))
  ...
  // Classify by signs on either side. Use a t-sample where f is non-degenerate
  // to determine the sign behavior; average across the working samples for
  // robustness against sin/cos zero-crossings of t.
  const eps = (yMax - yMin) * 1e-3
  const tForClassify = tSamples.filter((_, i) => i < rootSets.length)
  return dedup.map(y0 => {
    let leftSum = 0, rightSum = 0, count = 0
    for (const t of tForClassify) {
      const l = evalF(code, t, y0 - eps)
      const r = evalF(code, t, y0 + eps)
    ...
    const left = count > 0 ? leftSum / count : 0
    const right = count > 0 ? rightSum / count : 0
    let kind
